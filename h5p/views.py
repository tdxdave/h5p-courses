...
from h5p.forms import (
    H5PFileForm,
    H5PUpdateForm,
)
from h5p.content import (
    h5p_file_process,
    process_update,
)
...
from .dependency import (
    get_all_library_js,
    get_all_library_css,
    get_all_editor_libraries,
    get_core_js,
    get_editor_js,
    get_core_css,
    get_editor_css,
    get_library_libraries,
...
def h5p_view_content(request, content_id):
    ...
    content.json_content = content.json_content.replace('\\', '\\\\').replace('"', '\\"')
    main_library = content.main_library
    all_libraries = []
    for lib in H5PContentLibrary.objects.filter(content=content).order_by('-weight'):
        all_libraries.append(lib.library)
    all_libraries.append(main_library)
    all_js = get_all_library_js(all_libraries)
    all_css = get_all_library_css(all_libraries)
    # TODO cache combined js/css for each library,
    # or overriding library for a content type?
    # or per content item?
...
def h5p_ajax_libraries(request):
    ...
    if request.GET.get('machineName'):
        l = H5PLibrary.objects.get(machine_name=request.GET.get('machineName'))
        semantics = json.loads(l.semantics)
        editor_libraries = get_library_libraries(l, 'editor')
        editor_deps = []
        for editor_lib in editor_libraries:
            lib = editor_lib.required_library
            editor_deps.append({'machine_name': lib.machine_name, 'majorVersion': lib.major_version, 'minorVersion': lib.minor_version})
        data = {'uberName': l.machine_name + ' ' + str(l.major_version) + '.' + str(l.minor_version), 'name': l.machine_name, 'majorVersion': l.major_version, 'minorVersion': l.minor_version, 'title': l.title, 'runnable': l.runnable, 'restricted': l.restricted, 'semantics': semantics, 'editorDependencies': editor_deps}
        # TODO we need to add javascript element which contains pairts of
        # js url and actual js content
        data['javascript'] = get_content_type_editor_scripts(l)
        data['css'] = get_content_type_editor_styles(l)
    else:
        if request.GET.get('libraries[]'):
            libraries = []
            get_libraries = request.GET.get('libraries[]')
            libraries.append(H5PLibrary.objects.get(machine_name=get_libraries.split()[0]))
        else:
            libraries = get_content_type_libraries()
            for l in libraries:
                one_lib = {'uberName': l.machine_name + ' ' + str(l.major_version) + '.' + str(l.minor_version), 'name': l.machine_name, 'title': l.title, 'majorVersion': l.major_version, 'minorVersion': l.minor_version, 'restricted': False, 'tutorialUrl': l.tutorial_url}
                data.append(one_lib)
